...
export async function manageTaskStatus() {
  await moveActivesToPending();
  await checkPendingsValues();
}
...
function validateTaskValue(task: Task, now: Date): number {
  let values;
  if (task.difficulty === "easy") values = [10, 5];
  if (task.difficulty === "medium") values = [17, 14, 10, 4];
  if (task.difficulty === "hard") values = [23, 21, 18, 14, 10, 6];

  // How many 24h has the task been pending for?
  const daysLate = Math.floor(
    (now.getTime() - task.deadline!.getTime()) / 86400000,
  );
